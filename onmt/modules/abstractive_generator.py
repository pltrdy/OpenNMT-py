...
class LossWrapper(LossComputeBase):
    ...
    def _compute_loss(self, *args, **kwargs):
        loss, stats, sent_loss, scores = self.base_loss._compute_loss(
            *args, ret_scores=True, **kwargs)
        wloss, wstats = self.wrap_loss(
            loss, stats, *args, scores=scores, **kwargs)
        # print(loss, wloss)
...
class AbstractiveLossCompute(LossWrapper):
    def __init__(self, base_loss, abstract_metric="cosine",
                 abstract_lambda=1.0, abstract_pen=None, abstract_ntoks=10):
    ...
    def copypen(self, loss, stats, batch, src_embs, tgt_embs,
                output, target, tgt_lens, *args, scores=None, **kwargs):
        """
        ...
        if True:
            ...
            assert logprobs.lt(0).all(), "logprobs>= 0 %s\n%s" % (
                str(logprobs[logprobs.ge(0)]), str(logprobs.ge(0).nonzero()))
            assert logprobs.gt(float(
                "-inf")).all(), "logprobs<=inf %s" % str(logprobs[logprobs.le(float("-inf"))])
            no_nan(logprobs, "logprobs")

            probs = logprobs.exp()
            assert probs.ge(0).all(), "probs<= 0 %s\n%s" % (
                str(probs[probs.le(0)]), str(probs.le(0).nonzero()))
            assert probs.le(1).all(), "probs>= 1 %s\n%s" % (
                str(probs[probs.ge(1)]), str(probs.ge(1).nonzero()))
            no_nan(probs, "probs")
            log("probs", probs)

            # SUM over time and NORMALIZE
            tot_probs = (probs * maskYs.unsqueeze(2).float()
                         ).sum(0) / ntokYs.unsqueeze(1).float()
            log("tot_probs", tot_probs)
            assert tot_probs.ge(0).all(), "tot_probs<= 0 %s\n%s" % (
                str(tot_probs[tot_probs.le(0)]), str(tot_probs.le(0).nonzero()))
            assert tot_probs.le(1).all(), "tot_probs>= 1 %s\n%s" % (
                str(tot_probs[tot_probs.ge(1)]), str(tot_probs.ge(1).nonzero()))
            ...
            badcopy_pen_probs = (1 - badcopy_probs) * \
                Xtoks_notin_Ys.t().gt(last_spetok).float()
            ...
            assert badcopy_pen.lt(1).all(), "badcopy_pen>=1 %s" % (
                str(badcopy_pen[badcopy_pen.ge(1)]))
            assert badcopy_pen.ge(0).all(), "badcopy_pen<=0 %s" % (
                str(badcopy_pen[badcopy_pen.le(0)]))
            ...
            log("loss", loss)
            no_nan(
                loss, "loss (normalized by %s [%s])" %
                (str(
                    X_notin_Ys.float().sum(0)), str(
                    X_notin_Ys.float())))
            ...
            assert penalty_probs.ge(0).all(), "pen prob <= 0 %s" % str(
                penalty_probs[penalty_probs.le(0)])
            assert penalty_probs.le(1).all(), "pen prob >= 1 %s" % str(
                penalty_probs[penalty_probs.ge(1)])
            ...
            assert tot_penalty_logprobs.lt(0).all(), "tot_penalty_logprobs>= 0 %s" % str(
                tot_penalty_logprobs[tot_penalty_logprobs.ge(0)])
            assert tot_penalty_logprobs.gt(float("-inf")).all(), "tot_penalty_logprobs<=inf %s" % str(
                tot_penalty_logprobs[tot_penalty_logprobs.le(float("-inf"))])
            ...
            badcopy_logprobs = tot_penalty_logprobs.gather(
                1, Xtoks_notin_Ys.t())
            ...
            assert badcopy_neglogprobs.lt(float("inf")).all(), "badcopy nelp inf, corresponding toks %s" % (
                str(Xtoks_notin_Ys.t()[badcopy_neglogprobs.lt.eq(float("inf"))]))
            assert badcopy_neglogprobs.gt(0).all(), "badcopy neglp 0, corresponding toks %s" % (
                str(Xtoks_notin_Ys.t()[badcopy_neglogprobs.le(0)]))
            ...
            # s = -log p => -log[1-exp(-s)] = -log(1-p) giving neg log probs
            # for 1-p
            ...
            if src_len_norm:
                ...
                no_nan(loss, "loss (not normalized) %s" %
                       (str(X_notin_Ys.float().sum(0))))

            # print("Loss Norm: %s\nLoss: %s" % (str(loss_norm), str(loss)))
        return loss

    def tokagg_mseloss(self, loss, stats, batch, src_embs,
                       tgt_embs, output, target, tgt_lens, *args, **kwargs):
        src, tgt = batch.src, batch.tgt
        src, src_lens = src
    ...
    def abstrpen(self, loss, stats, batch, src_embs, tgt_embs,
                 output, target, tgt_lens, *args, **kwargs):
        def align(X, Y):
        ...
        for i in range(b):
            ...
            penalties[:, i, :] = penalties[:, i,
                                           :].index_fill(-1, src[:, i, 0], penalty)
        scores = scores * penalties
        loss = torch.nn.functional.nll_loss(self._bottle(
            scores), tgt.view(-1), ignore_index=self.padding_idx, reduction='sum')
        return loss

    def inv_abstrpen(self, loss, stats, batch, src_embs,
                     tgt_embs, output, target, tgt_lens, *args, **kwargs):
        def align(X, Y):
            """Given two matrices X [n x b], Y [m x b]
               Return mask of Y being in X
    ...
    def abstrtok(self, loss, stats, batch, src_embs, tgt_embs,
                 output, target, tgt_lens, *args, **kwargs):
    ...
    def abstrrate(self, loss, stats, batch, src_embs, tgt_embs,
                  output, target, tgt_lens, *args, **kwargs):
